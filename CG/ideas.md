...
- 要解决的问题: 
  ...
  - 再通过图像修复和超分辨率技术，填补因为结构缺失造成的纹理空洞，提高纹理的清晰度，为简化重建的模型还原相片级别的纹理细节，在保证性能的同时提供超真实感的画面，提高实时应用的沉浸度
...
- ![image-20210618140236731](ideas.assets/image-20210618140236731.png)
  1. 通过区域增长和平面拟合得到中间结果，在这过程中会产生一些outliers，可以在原始模型中标记这些outliers，代表此处有很大的结构差异。生成的中间结果是一个alpha shape集合，每个alpha shape中的极点映射回原始模型就是原始模型的角点。
...
- 建立角点映射
  ...
  - why: 简化重建过程中，顶点位置会发生偏移，将简化模型和原始模型的顶点从三维空间变换到图像空间后这些点会发生错位，因此我们需要建立原始模型与简化模型之间的角点映射，在下一步做纹理映射时能将简化模型的点正确变换到图像空间中

  ![image-20210618134728050](ideas.assets/image-20210618134728050.png)

  - 在简化重建过程中会用<u>区域增长法</u>对冗余模型中的面片进行聚类并拟合成平面，在该过程中能标记哪些面片是被拟合的，哪些面片是被作为离群值删除的，在区域增长拟合完平面后能获得冗余模型与简化模型的一个初始映射
  - 之后简化重建会使用二叉空间分割（Binary space partitioning, BSP）对空间进行分割，最后在分割的基础上根据内外信息提取模型表面，最终获得简化模型。
  - 在进行空间分割的过程中冗余结构被进一步去除，可以利用分割结果增强初始的映射关系，最后在冗余模型上完成标记
...
- 曲面细分
  ...
  - 在下一步进行纹理映射时，因为要对面片建立马尔科夫随机场，而简化模型的面片数量很少，很难对映射过程做一个比较好的约束，因此在映射前需要**对简化模型的参数化平面进行细分**

- 约束纹理映射

  - 根据上一步得到的映射关系对纹理映射过程做一个约束
