...
- 要实现的:
  - 通过建立原始重建结果与简化重建结果之间的映射关系，自适应地将高分辨率图像中被保留下来的结构的纹理信息映射到简化三维模型上.
  - 通过图像修复和超分辨率技术，填补因为结构缺失造成的纹理空洞，提高纹理的清晰度，为简化重建的模型还原相片级别的纹理细节，在保证性能的同时提供超真实感的画面，提高实时应用的沉浸度
- 纹理映射需要频繁地将三维空间的点或面变换到图像空间，因此需要三维重建的结果非常精确，所以不能直接对简化模型做纹理映射
- 存在的问题:
  - 存在几何差异: 简化模型有结构缺失, 将缺失的结构的纹理映射到简化模型上会出问题
...
> 建立原始模型(冗余模型)与简化模型之间的映射

### 建立角点映射

> 简化重建过程中，顶点位置会发生偏移，将简化模型和原始模型的顶点从三维空间变换到图像空间后这些点会发生错位，因此需要建立原始模型与简化模型之间的角点映射，保证在下一步做纹理映射时能将简化模型的点正确变换到图像空间中
>
> 根据角点对视角图片进行形变, 再进行映射

- 基于Kinetic shape reconstruction的简化重建结果, 需要根据这个方法找到哪些结构是缺失的, 哪些是额外增加的, 哪些点是相对应的, 从而建立原始模型与简化模型结果之间的映射

<img src="ideas.assets/image-20210618140236731.png" alt="image-20210618140236731" style="zoom:80%;" />

1. 通过区域增长?和平面拟合?得到中间结果，在这过程中会产生一些outliers(被简化掉的部分)，可以在原始模型中标记这些outliers，代表此处有很大的结构差异, 之后就不会给这部分映射纹理。生成的中间结果是一个alpha shape集合?，每个alpha shape中的极点映射回原始模型就是原始模型的**角点**
   - 用<u>区域增长法</u>对原始模型中的面片进行聚类并拟合成平面，在该过程中能标记哪些面片是被拟合的，哪些面片是被作为离群值删除的，在区域增长拟合完平面后能获得原始模型与简化模型的一个初始映射
2. 通过BSP和表面提取得到最终结果，通过同样的方法可以得到中间结果到简化模型的outliers和角点映射，最终建立原始模型与简化模型的映射?
   - 使用二叉空间分割（Binary space partitioning, BSP）对空间进行分割?，最后在分割的基础上根据内外信息提取模型表面，最终获得简化模型
   - 在进行空间分割的过程中冗余结构被进一步去除?，可以利用分割结果增强初始的映射关系，最后在原始模型上完成标记

### 标记结构差异

<img src="ideas.assets/image-20210618134922671.png" alt="image-20210618134922671" style="zoom:80%;" />
...
- why: Prior work, which typically operate by assembling planar shapes detected from input points, largely overlooked the scalability issue of processing a large number of shapes. As a result, they tend to produce overly simplified meshes with assembling approaches that can hardly digest more than one hundred shapes in practice. 先前的工作通常通过组装从输入点检测到的平面形状来操作，在很大程度上忽略了处理大量形状的可扩展性问题。结果就是他们倾向于使用组装方法生成过于简化的网格，在实践中很难消化超过一百个形状
- Our key idea relies upon the design of a kinetic data structure for partitioning the space into convex polyhedra. 关键思想依赖于将空间划分为凸多面体的动力学数据结构的设计
- partitioning the 3D space into convex polyhedra
- our formulation relies upon a fast voting scheme that leverages the orientation of point normal.
- Instead of slicing all the planar shapes exhaustively as prior methods, we create a partition where shapes grow at constant speed until colliding and forming polyhedra.  没有像以前的方法那样详尽地切割所有平面形状，而是创建一个分区，其中形状以恒定速度增长，直到碰撞并形成多面体

#### kinetic data structure

- is composed of a set of geometric primitives whose coordinates are continuous functions of time

#### kinetic frameworks

- purpose: maintain the validity of a set of geometric properties on the primitives, also called certificates
...
[link](https://graphics.stanford.edu/courses/cs268-11-spring/handouts/AlphaShapes/as_fisher.pdf)

- A very small value will allow us to eat up all of the ice-cream except the chocolate points S themselves. Thus we already see that the α-shape of S degenerates to the point-set S for α → 0. α太小, alpha shape 退化为S点集
- A huge value of α will prevent us even from moving the spoon between two points since it’s way too large. So we will never spoon up ice-cream lying in the inside of the convex hull of S, and hence the α-shape for α →∞ is the convex hull of S. α太大, 得到S的一个凸包

## 纹理映射

> 将超高分辨率图片的颜色信息映射到简化后的三维模型上

- 在纹理映射过程中，需要将像素从图像空间变换到三维空间中，根据冗余模型上的标记判断该像素对应的面片是否已经被删除，防止将被删除结构的纹理映射到简化模型上
- 如果简化模型相比原始模型多了一些额外的结构，就要避免将颜色信息映射在这些结构上，因为这些结构的纹理信息不存在于任何视角中，后续我们再用其他方法修复
- 在纹理映射结束后可以得到一个粗糙的带纹理的简化模型

### 曲面细分

> 在进行纹理映射时，因为要对面片建立马尔科夫随机场，而简化模型的面片数量很少，很难对映射过程(视角选取)做一个比较好的约束? 因此在映射前需要**对简化模型的参数化平面进行曲面细分**，提升纹理映射的准确率

![image-20210618135016819](ideas.assets/image-20210618135016819.png)

### 约束纹理映射

- 根据上一步得到的映射关系对纹理映射过程做一个约束, 有选择地将纹理信息还原到简化模型上
...
- 图像校正?
  ...
  - Seamless Color Mapping for 3D Reconstruction with Consumer-Grade Scanning Devices
    ...
    ![image-20210619130132365](ideas.assets/image-20210619130132365.png)

## 纹理修复

### 修补孔洞

> 因为结构差异，在进行纹理映射时需要抛弃一些纹理信息，因此纹理会留下一些不规则的孔洞, 需要修复

- Liu等人(Image Inpainting for Irregular Holes Using Partial Convolutions)提出了基于部分卷积的图像修复，在对不规则孔洞的修复上取得了很好的效果，之后可能会用该方法修复纹理

### 全局纹理优化

> 对纹理进行补全后，需要对纹理进行优化，消除纹理映射和修复过程中产生的裂缝，模糊等现象

## 超分辨率

> 提高3D表面纹理清晰度
>
> 优化后的纹理可能因为平滑导致分辨率有所下降，因此需要使用超分辨率技术提升纹理的清晰度，还原更更丰富的纹理细节

- ![image-20210619131021376](ideas.assets/image-20210619131021376.png)

  Li等人基于2D图像超分辨率方法，通过结合3D物体的法向信息，提出了一个更适合3D物体的纹理超分辨率方法
